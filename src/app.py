...
with gr.Blocks(theme="darkhuggingface") as demo:
    ...
    This app combines a supervised learning Named Entity Recognition
    (NER) with Microsoft Presidio. The combination of a pre-trained
    Spacy `en_core_web_lg` and Presidio provides a comprehensive
    solution for detecting and anonymizing PII in text data.

    The next version of this app will use `deid_roberta_i2b2` instead of
    Spacy's `en_core_web_lg`. `deid_roberta_i2b2` is a supervised model
    that excels at detecting PHI/PII entities. This approach is in
    compliance with privacy laws like HIPAA, ensuring sensitive personal
    and health information remains secure and confidential.
    """
    )
